...
def generate_plan_auto(query_text: str, intent_result: Dict[str, Any]) -> List[Dict[str, Any]]:
    # Plan stays template-based; intent may come from LLM
    plan = generate_plan(query_text, intent_result)
    _enrich_plan_from_entities(plan, intent_result.get("entities") or [])
    return plan


def _enrich_plan_from_entities(plan: List[Dict[str, Any]], entities: List[Dict[str, Any]]) -> None:
