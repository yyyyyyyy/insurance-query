...
@dataclass
class CacheEntry:
    key: str; value: Any; trace_id: str = ""
    created_at: float = field(default_factory=time.time)
    ttl_seconds: float = 300.0; hit_count: int = 0
    @property
    def expired(self) -> bool: return (time.time() - self.created_at) > self.ttl_seconds
...
class TraceAwareCache:
    ...
    def get(self, store: str, key: str) -> Tuple[Optional[Any], bool]:
        if store not in self._stores: return None, False
        e = self._stores[store].get(key)
        if e is None: self._stats[store]["misses"] += 1; return None, False
        if e.expired: self._stats[store]["evictions"] += 1; del self._stores[store][key]; return None, False
        e.hit_count += 1; self._stats[store]["hits"] += 1; return e.value, True

    def set(self, store, key, value, trace_id="", ttl=None):
        if store not in self._stores: return
    ...
    def invalidate(self, store=None):
        count = 0
        if store and store in self._stores: count = len(self._stores[store]); self._stores[store].clear()
        elif store is None:
            for s in self._stores: count += len(self._stores[s]); self._stores[s].clear()
        return count
